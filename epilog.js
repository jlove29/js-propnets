...
function baseapplyrs (fun,args,facts,rules)
 {var result = seq(fun).concat(args);
  ...
  /* STUFF I ADDED */
function regbasefinds (result,query,facts,rules)
 {var answers = seq();
  baseall(result,query,seq(),seq(),nil,answers,facts,rules);
  return uniquify(answers)}

function regbasefindp (query,facts,rules)
 {
 return (basefindx('true',query,facts,rules)==='true')}

function regbasefindx (result,query,facts,rules)
...
var builtins =
 ["hastype","plus","minus","times","quotient",
...
// reading//==============================================================================
  ...
  if (input.charCodeAt(cur+1)===45) {output[output.length] = ':-'; return cur+2};
  if (input.charCodeAt(cur+1)===61) {output[output.length] = ':='; return cur+2};     throw 'error'}function scanthussym (cur) {if (input.length>cur+2 && input.charCodeAt(cur+1)===61
